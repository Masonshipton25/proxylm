...
df = pd.read_csv('src\proxy_regressor\csv_datasets\mt560_experiments.csv', header=None, usecols=columns_to_extract)
#89:95
...
for pair in pairs:
    ...
    counter += 1
# Read the CSV file without headers
df3 = pd.read_csv('src\proxy_regressor\csv_datasets\mt560_experiments.csv')
...
# columns_to_extract = [2, 3, 4, 5, 6, 7, 8]  # For example, to extract the first and third columns (0-based index)
# # Read the CSV file without headers
# df = pd.read_csv('nusa_distances.csv', header=None, usecols=columns_to_extract)
# # Skip the first row if it contains headers
# df = df.iloc[1:]
# # Convert specified columns to lists
# distances = {
...
#     counter += 1
# df2 = pd.read_csv('src\\proxy_regressor\\csv_datasets\\new_nusa_experiments.csv')

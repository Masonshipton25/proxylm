#All databases, union aggregated, imputed with SoftImpute.
from lang2vec import URIEL2 as uriel
import csv
from lang2vec import lang2vec as l2v

#Integrate all databases if they have not been included.
uriel.update_ALL()
...
langs = [MT560_langs, NUSA_langs]

distances = ['genetic','geographic','syntactic','inventory','phonological','featural','morphological']
headers = ['source_lang','target_lang'] + distances

files = ["distances/mt560_distances.csv", "distances/nusa_distances.csv"]

for i, file in enumerate(files):
    with open(file, 'w', encoding='utf-8', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(headers)
        for lang_pair in langs[i]:
            distance_values = l2v.new_distance(distances, lang_pair, union_or_average='U', fill_with_base_lang_param=True)
            
            row = lang_pair + distance_values
            print(row)

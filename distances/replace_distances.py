...
df = pd.read_csv('experiment_csvs\\URIEL++\\without_scriptural\\mt560_experiments.csv', header=None, usecols=columns_to_extract)
#89:95
...
for pair, count in pair_counts.items():
    pairs.append(pair)
# Specify the columns you want (by index or name)
columns_to_extract = [2, 3, 4, 5, 6, 7, 8, 9]  # For example, to extract the first and third columns (0-based index)
# Read the CSV file without headers
df2 = pd.read_csv('distances\cosine_mt560_distances.csv', header=None, usecols=columns_to_extract)
...
for pair in pairs:
    ...
    counter += 1
# Read the CSV file without headers
df3 = pd.read_csv('experiment_csvs\\URIEL++\\without_scriptural\\mt560_experiments.csv')
    ...
    for pair in pairs:
        ...
        if row[0] == pair[0] and row[1] == pair[1]:
            ...
            if end_col <= len(df3.columns):
                ...
                scriptural_column.append(distances['Column_7'][counter])
                break
            else:
...
df3['scriptural'] = scriptural_column
# Save the modified DataFrame back to a CSV file
df3.to_csv('src\\proxy_regressor\\csv_datasets\\new_mt560_experiments.csv', index=False)
...
# columns_to_extract = [2, 3, 4, 5, 6, 7, 8, 9]  # For example, to extract the first and third columns (0-based index)
# # Read the CSV file without headers
# df = pd.read_csv('distances\\cosine_nusa_distances.csv', header=None, usecols=columns_to_extract)
# # Skip the first row if it contains headers
# df = df.iloc[1:]
# # Convert specified columns to lists
# distances = {
...
# distances_dict = {}
# counter = 0
# for category in ['genetic', 'geographic', 'syntactic', 'inventory', 'phonological', 'featural', 'morphological', 'scriptural']:
#     specific_distances = []
